...
We will call the stacks $T$ and $R$. Initially, set the variables $T.top = 0$ and $R.top = n+1$.  Essentially, stack $T$ uses the first part of the array and stack $R$ uses the last part of the array.  In stack $T$, the top is the rightmost element of $T$.  In stack $R$, the top is the leftmost element of $R$. 
\begin{algorithm}
\caption{PUSH(S,x)}
\begin{algorithmic}[1]
\If{$S == T$}
