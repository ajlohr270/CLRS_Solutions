...
%We know that there is a finite-capacity cut, because we could simply cut the source from all of the nodes it is adjacent to, which has capacity $\sum_i a_i$. This means that the minimum cut must be finite, so, by the previous part, the cut must 
