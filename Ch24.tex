...
%To solve the problem of $Ax \leq b$ where the elements of $b$ are real-valued is simple.  We carry out the same procedure as before, but allow our edge weights to be real-valued. To impose the integer condition, we modify the RELAX procedure.   Suppose we are going to relax $v_i$.  If it is required to be an integer, then we relax it to ...
