...
Condition one of being a matroid is still satisfied because the base set hasn't changed. Next we show that $\mathcal{I}'$ is nonempty. Let $A$ be any maximal element of $\mathcal{I}$ then, we have that $S - A \in \mathcal{I}'$ because $S - (S-A) = A\supseteq A$ and $\mathcal{I}$ is hereditary. Next we show the hereditary property of $\mathcal{I}'$, suppose that $B \subseteq A \in \mathcal{I}'$, then, there exists some maximal $A'\in\mathcal{I}$ so that $S - A \supseteq A'$, However, $S-B \supseteq S-A \supseteq A'$, so $B\in \mathcal{I}'$.

Lastly we prove the exchange property. That is, if we have $B,A \in \mathcal{I}'$ and $|B|<|A|$ we can find an element $x$ in $A-B$ to add to $B$ so that it stays independent. Let $A'$ be a maximal independent set of $\mathcal{I}$ contained in $S-A$ and Let $B'$ be a maximal independent set of $\mathcal{I}$ contained in $S-B$. Since $A'$ and $B'$ are maximal in $\mathcal{I}$, we must have that $|A'|=|B'|$ by theorem 16.6. Since $|A'| \le |S-A| < |S-B|$, this means that $|S-B| > |B'|$. So, we can pick some $x \in (S-B) -B'$. Then, we will still have that $B'\subseteq S-(B\cup\{x\})$. Restating this, we have extended $B$ by a single element $x$ without loosing that its complement contained $B'$, hence, $B\cup\{x\}$ is an independent set of $\mathcal{I}'$. 	
